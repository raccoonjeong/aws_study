...
* 액세스 키 보안 관련
```
액세스 키 자체는 시스템이 작동할 때 필요한 정보이므로 서버상에 프로그램과 함께 저장하는 경우가 많다. 따라서 서버 자체의 보안을 반드시 고려해야 한다. 그러나 서버를 보호하더라도, 프로그래머가 액세스 키를 깃허브 (GitHub) 등 소스 관리 시스템의 저장소에 업로드하거나 스마트폰 애플리케이션 안에 삽입하는 경우도 있다. 이렇게 인프라스트럭처 담당자가 서버를 보호해도 액세스 키가 누출되는 경우가 발생한다.
이처럼 액세스 키를 불법으로 입수한 사람이 수많은 EC2 인스턴스를 생성해 비트코인 등의 가상 화폐를 채굴하는 사례가 최근 증가했다. 그 결과 한 달에 수백만 원에서 수천만 원의 이용료가 발생하기도 한다.
누출이 발생했을 때는 먼저 액세스 키를 삭제해야 한다. 그리고 잘못 실행된 리소스도 모두 삭제한다. 네트워크상 에서는 AWSOII 이용료를 면제받은 경우도 찾아볼 수 있다. 그러나 반드시 면제를 받을 수 있는 것은 아니므로 액세스 키는 절대 누출되어서는 안 된다.
프로그래머들도 부주의하게 액세스 키를 소스 코드 관리 도구의 관리 대상으로 설정하거나 애플리케이션 리소스로 삽입하지 않아야 한다. 특히 AWS는 액세스 키 등이 깃허브와 같은 저장소에 업로드되는 것을 방지하는 ‘gitsecrets’라는 도구(https：//github.com/awsl3bs/git-secrets)를 제공하므로 이를 적극적으로 이용한다.
```
